    ...
    for j in range(2):
        ...
        for k in range(len(maxline_levels)):
            if k < idx_begin or k > idx_end:
                continue
            axes[i].plot((maxline_t[k, 0:maxline_levels[k]]-(MOX_DAQ_FREQ*MOX_INTERP_FACTOR*20-LEN_WAVELET+LEN_WAVELET/2+SIGNAL_DELAY*MOX_DAQ_FREQ*MOX_INTERP_FACTOR/2))/(float)(MOX_DAQ_FREQ*MOX_INTERP_FACTOR), range(maxline_levels[k]), linewidth=1, color='k')
    axes[i].set_xlabel('time (s)')
    axes[i].set_ylabel('scales \nof sensor '+str(i+1))

plt.savefig('maxline_scale.pdf', format='pdf', bbox_inches="tight")
